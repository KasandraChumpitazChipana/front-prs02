...
const Teachers = () => {
  ...
  // Function to load teachers, now dependent on statusFilter
  const loadTeachers = useCallback(async () => {
      setLoading(true);
      setError(null);
      try {
          // Use the teacherService to fetch data, using the statusFilter from state
          const data = await teacherService.getAllTeachers(statusFilter || null);
  ...
  }, [statusFilter]); // Dependency array now includes statusFilter

  // Effect to load teachers when the component mounts or loadTeachers function changes (due to statusFilter change)
  useEffect(() => {
    loadTeachers();
  }, [loadTeachers]);
  ...
  const handleEditTeacher = (teacher) => {
    ...
    navigate(`/directors/teachers/edit/${teacherId}`); // Navigate to the edit form route with ID
  };

  const handleViewTeacher = (teacher) => {
    const teacherId = teacher.id || teacher._id;
    navigate(`/directors/teachers/view/${teacherId}`); // Navigate to the view page route with ID
  };
    ...
    const handleStatusChange = async (id, currentStatus) => {
        ...
        try {
            ...
            loadTeachers(); // Refresh list after status change
        } catch (error) {
